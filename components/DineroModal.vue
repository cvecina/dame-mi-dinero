<template>
  ...
  >
    <div
      class="bg-blanco-dividido rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto"
    >
      <!-- Header -->
      <div
      ...
      <div class="p-6">
        ...
        <div v-if="dineroStore.isLoading" class="text-center py-8">
          <div
            class="w-8 h-8 border-2 border-azul-tiquet border-t-transparent rounded-full animate-spin mx-auto mb-2"
          ></div>
            ...
            <div class="space-y-3">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  ></div>
                  <div>
                    <h4 class="font-medium text-gris-billetera">
                      {{ dinero.name }}
                    </h4>
                    <p class="text-sm text-gray-600">
                      {{ getExpenseCount(dinero.id) }} gastos
                    </p>
                    <span
                      v-if="dinero.isDefault"
                      class="text-xs px-2 py-1 bg-azul-tiquet/10 text-azul-tiquet rounded-full"
                    >
                      Por defecto
                    </span>
                  </div>
          ...
          <div class="border-t border-gray-200 pt-6">
            ...
            <form @submit.prevent="saveDinero" class="space-y-4">
              ...
              <div class="flex gap-3 pt-4">
                <div v-if="Array.isArray(form.users)">
                  <label class="block text-sm font-medium text-gray-700 mb-2"
                    >Usuarios asociados</label
                  >
                  <div class="flex flex-wrap gap-2 mb-2">
                    ...
                    >
                      <label
                        class="flex items-center gap-2 bg-marfil-mapamundi px-3 py-2 rounded-lg cursor-pointer"
                      >
                        <input
                          type="checkbox"
                          v-model="form.users"
                          :value="user.id"
                          class="accent-azul-tiquet"
                        />
                        <span class="text-gris-billetera text-sm">{{
                          user.name
                        }}</span>
                      </label>
                    </template>
...
const editDinero = (dinero) => {
  ...
  form.value.name = dinero.name;
  form.value.description = dinero.description || "";
  form.value.color = dinero.color;
  form.value.users = Array.isArray(dinero.users) ? [...dinero.users] : [];
};
...
const saveDinero = async () => {
  loading.value = true;
  try {
    // Convertir form a objeto plano para evitar refs/reactivity
    const plainForm = JSON.parse(JSON.stringify(form.value));
    if (editingDinero.value) {
      await dineroStore.updateDinero(editingDinero.value.id, plainForm);
      alertStore.success("Dinero actualizado correctamente");
    } else {
      await dineroStore.addDinero(plainForm);
...
const deleteDinero = async (dineroId) => {
  if (
    !confirm(
      "¿Estás seguro de que quieres eliminar este dinero? Los gastos asociados se moverán al dinero por defecto."
    )
  ) {
    return;
  }
